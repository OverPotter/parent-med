...
@router.post("/test/pillbox", response_model=PushNotificationTestResponseDto)
async def send_pillbox_test_push_notification(
    current_account: AuthenticatedAccount = Depends(get_current_account),
    service: PushNotificationService = Depends(get_push_notification_service),
) -> PushNotificationTestResponseDto:
    if not settings.debug:
        raise HTTPException(status_code=404, detail="Not Found")
    scheduler = get_push_scheduler()
    return await service.send_pillbox_test_notification(current_account.id, scheduler)
