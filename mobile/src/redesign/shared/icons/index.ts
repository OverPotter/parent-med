export const redesignSharedIcons = {
  feeding: require("./bottle_transparent.png"),
  sleep: require("./moon_sleep_icon_transparent.png"),
  profile: require("./profile_icon_transparent.png"),
  observation: require("./thermometer_transparent.png"),
  illnessBadge: require("./pastel_thermometer_badge_icon_transparent.png"),
  height: require("./height_measurement_icon_on_lavender_circle_transparent.png"),
  overview: require("./soft_blue_list_icon_with_neumorphism_transparent.png"),
} as const;
