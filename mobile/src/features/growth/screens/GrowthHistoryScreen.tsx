...
import {
  createMobileHeightEntry,
  deleteMobileHeightEntry,
  fetchMobileHeightEntries,
  type MobileHeightEntry,
} from "../api/heightEntriesApi";
import {
...
import { MetricColumn, TimelineRow } from "./GrowthHistoryScreenParts";
import {
  buildHeightDeleteDialogCopy,
  buildHeightDeleteErrorCopy,
  buildHeightMeasurementSheetCopy,
  parseHeightValue,
} from "./growthHistoryScreenCopy";
...
export function GrowthHistoryScreen({
  ...
  const [activePeriodId, setActivePeriodId] = useState(
    buildGrowthHistoryScreenContent(locale, childDisplayName).periods.find(
      (item) => item.active,
    )?.id ?? "all",
  );
  const [entries, setEntries] = useState<MobileHeightEntry[]>([]);
  ...
  const measurementSheetCopy = buildHeightMeasurementSheetCopy(locale);
  const deleteDialogCopy = buildHeightDeleteDialogCopy(locale);
  const deleteErrorCopy = buildHeightDeleteErrorCopy(locale);
  ...
  useEffect(() => {
    ...
    async function loadTimeline() {
      try {
        const items = await fetchMobileHeightEntries(authSession, child.child.id);
  ...
  const filteredEntries = useMemo(() => {
    ...
    return filterHeightEntriesByPeriod(entries, activePeriodId);
  }, [activePeriodId, customRange, entries]);
  const content = useMemo(
    () => buildGrowthHistoryScreenContent(locale, childDisplayName, activePeriodId),
    [activePeriodId, childDisplayName, locale],
  );
  const timeline = useMemo(
    () => mapHeightTimelineFromApi(filteredEntries, locale),
    [filteredEntries, locale],
  );
  const metrics = useMemo(
    () => buildGrowthMetricsFromApi(filteredEntries, locale),
    [filteredEntries, locale],
  );
  ...
  return (
    <View style={styles.screenRoot}>
      <JournalScreenScaffold
        ...
        onSelectPeriod={(id) => {
          setCustomRange(null);
          setActivePeriodId(id as typeof activePeriodId);
        }}
        onBack={onBack}
            ...
            <View style={styles.sheetBody}>
              <View style={styles.sheetField}>
                <Text style={styles.sheetFieldLabel}>
                  {measurementSheetCopy.heightLabel}
                </Text>
                <View style={styles.sheetInputWrap}>
                  <TextInput
                    ...
                    placeholder={measurementSheetCopy.heightPlaceholder}
                    placeholderTextColor="#98A7AB"
                    keyboardType="decimal-pad"
                    autoFocus
                  />
                  <Text style={styles.sheetInputSuffix}>cm</Text>
                </View>
              </View>
  ...
  async function handleSaveMeasurement() {
    const parsedValue = parseHeightValue(measurementValue);

    if (parsedValue === null || parsedValue <= 0) {
      setMeasurementError(measurementSheetCopy.invalidHeight);
      return;
    }

    try {
      setIsSavingMeasurement(true);
      const created = await createMobileHeightEntry(authSession, {
  ...
  async function handleDeleteEntry() {
    ...
    try {
      await deleteMobileHeightEntry(authSession, pendingDeleteEntryId);
      setEntries((current) =>
