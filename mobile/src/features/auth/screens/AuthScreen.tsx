...
type FieldTouchedState<T extends string> = Partial<Record<T, boolean>>;
type FocusedFieldId =
  | "email"
  | "password"
  | "passwordConfirm"
...
type AuthValidationErrors = Partial<Record<AuthFieldId, string>>;
...
function normalizeFormValue(key: keyof FormState, value: string) {
  return key === "familyCode" ? normalizeFamilyCodeInput(value) : value;
}

function getFieldStyle(field: AuthFieldConfig, fieldsLength: number) {
  if (fieldsLength === 1) {
...
function buildAuthFormErrors(
  formState: FormState,
  isRegisterMode: boolean,
  messages: ReturnType<typeof buildAuthScreenContent>["errors"],
...
function buildForgotPasswordErrors(
...
function findFirstVisibleError<T extends string>(
  orderedFields: T[],
...
function resolveKeyboardOffset(
                    ...
                    {fields.map((field) => {
                      ...
                      return (
                        <AuthInputField
                          ...
                          value={value}
                          error={error}
                          fieldStyle={getFieldStyle(field, fields.length)}
