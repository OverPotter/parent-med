export type VerifiedFamilyCode = {
  token: string;
  familyName: string;
  expiresAt: string;
};

export type FamilyCodePreviewPayload = {
  familyName: string;
  expiresAt: string;
};
...
export function normalizeFamilyCodeInput(value: string) {
  return value.replace(/\s+/g, "").trim();
}

export function shouldAutoVerifyFamilyCode(value: string) {
